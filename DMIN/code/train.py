...
def eval(sess, test_data, model, model_path, maxlen=20):
...
def train(
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        valid_data = DataIterator(valid_file, uid_voc, mid_voc, cat_voc, batch_size, maxlen)
        ...
        sys.stdout.flush()
        print('                                                                                      test_auc: %.4f ---- test_loss: %.4f ---- test_accuracy: %.4f ---- test_aux_loss: %.4f' % eval(sess, test_data, model, best_model_path, maxlen))
        sys.stdout.flush()
        ...
        for itr in range(3):
            ...
            for src, tgt in train_data:
                ...
                if (iter % test_iter) == 0:
                    ...
                    print('                                                                                          test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f ---- test_aux_loss: %.4f' % eval(sess, valid_data, model, best_model_path,maxlen=maxlen))
                    loss_sum = 0.0
                    accuracy_sum = 0.0
                    aux_loss_sum = 0.0
...
def test(
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f ---- test_aux_loss: %.4f' % eval(sess, test_data, model, model_path,maxlen=maxlen))
